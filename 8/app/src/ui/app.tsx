...
export function App() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (_accounts && _accounts[0]) {
                ...
                console.log('depositAddress: ' + depositAddress.addressString);
                setL2DepositAddress(depositAddress.addressString);


                refreshL2Balance();

                setInterval(() => { refreshL2Balance(); }, 5000);

                refreshSudtBalance();
                
                setInterval(() => { refreshSudtBalance(); }, 30000);
            }
            ...
            async function refreshL2Balance() {
                ...
                console.log('Refreshed L2 Balance: ' + _l2Balance);
            }

            async function refreshSudtBalance() {
                const addressTranslator = new AddressTranslator();
                const proxyContract = new _web3.eth.Contract(ERC20JSON.abi, CONFIG.ERC20_PROXY_CONTRACT_ADDRESS);
                const polyjuiceAddress = addressTranslator.ethAddressToGodwokenShortAddress(_accounts[0]);
                const _sudtBalance = await proxyContract.methods.balanceOf(polyjuiceAddress).call( { from: account } );
                
                setSudtBalance(_sudtBalance);

                console.log('Refreshed SUDT balance: ' + _sudtBalance);
            }
    ...
    return (
        <div>
            ...
            <br />
            SUDT Balance:{' '}
            <b>{sudtBalance ? sudtBalance : <LoadingIndicator />}</b>
            <br />
            <br />
            Your address for depositing to Layer 2
            <br />
            <textarea rows="12" cols="36" readOnly value={l2DepositAddress ? l2DepositAddress : 'Generating...'}></textarea>
            <br />
            Please ensure that the recipient is address is the same as above
            <br />
            <button disabled={!l2DepositAddress} onClick={(e) => 
                {
                    e.preventDefault();
                    window.open('https://force-bridge-test.ckbapp.dev/bridge/Ethereum/Nervos?recipient=' + l2DepositAddress + '&xchain-asset=0x0000000000000000000000000000000000000000', '_blank');
                }
            }>

            Force Bridge Deposit
            </button>
            <br />
            <br />
            <br />
